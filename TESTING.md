...
organised as a **test pyramid** and following **AAA** (Arrange → Act → Assert).

| Layer | Count | Tool | What it checks |
|-------|-------|------|----------------|
| **Unit** | 16 | pytest | `hash_password` / `verify_password` / `session_cookie_flags` / `get_session_token` in isolation (no DB, no network) |
| **Integration** | 12 | pytest + Flask test client | sign-up / login / logout / `require_auth` through the real endpoints |
| **E2E** | 1 | Cypress | sign up → log out → log in → view profile, in a real browser |
...
```powershell
cd server
py -m pytest -v                                    # all 28 backend tests
...
- The spec uses a **unique email per run**, so it stays repeatable against the
  persistent MySQL database (each run leaves one throwaway user — harmless).
- Selectors use `data-testid` attributes added to the auth form fields and nav.
